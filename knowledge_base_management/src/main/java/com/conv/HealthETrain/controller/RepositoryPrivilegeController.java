...
public class RepositoryPrivilegeController {
    ...
    */
        ...
        if (visibility == 0) {
            ...
            if (isVisibilitySuccess) {
                ...
                if (isAvailable) {
                    ...
                    log.info("设置知识库" + repositoryId + "全局权限为" + privilegeId + "失败");
                    return ApiResponse.error(NOT_MODIFIED);
                }
            }
        }
    ...
    public ApiResponse<Boolean> editUserListPrivilege(
        ...
        //设置visibility为公开
        Boolean isVisibilitySuccess = repositoryService.updateRepositoryVisibility(repositoryId, 1);
        if (isVisibilitySuccess) {
            log.error("设置知识库权限失败" + repositoryId);
            return ApiResponse.error(NOT_MODIFIED);
        } else {
            // 2.添加全部用户到link表里面
            //修改当前存在在权限表的用户权限
            List<UserRepositoryPrivilege> userNotePrivileges = userRepositoryPrivilegeService.findUserRepositoryPrivilegeByRepositoryId(repositoryId);
            List<Long> existUserIdList = userNotePrivileges.stream().map(UserRepositoryPrivilege::getUserId).toList();
            //修改存在在existUserIdList的权限，剩余的删除
            Boolean isAvailable = userRepositoryPrivilegeService.
                    updateAndDeletePrivilegeByRepositoryIdAndUserIdList(repositoryId, privilegeId, existUserIdList);
            if (isAvailable) {
                //添加其他用户进入
                List<Long> userIdList = userList.stream().map(User::getUserId).toList();
                //修改剩余的权限
